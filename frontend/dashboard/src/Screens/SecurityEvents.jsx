...
export default function SecurityEvents() {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-[#1E2939] to-[#0F172A] p-4 text-slate-100 sm:p-6 lg:p-8"><div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/70"><div className="flex flex-col gap-3 border-b border-slate-800 p-4 sm:flex-row sm:items-center sm:justify-between"><select value={filter} onChange={(event) => setFilter(event.target.value)} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2"><option value="">전체 유형</option><option value="USB_CONNECTED">USB 연결</option><option value="USB_DISCONNECTED">USB 해제</option><option value="USB_FILE_WRITTEN">USB 파일 생성·변경</option><option value="FILE_COPY">파일 복사</option><option value="FILE_CREATED">파일 생성</option><option value="FILE_MODIFIED">파일 수정</option><option value="FILE_DELETED">파일 삭제</option><option value="FILE_MOVED">파일 이동</option><option value="NETWORK_CONNECTION">외부 네트워크 연결</option><option value="NETWORK_TRANSFER">확인된 네트워크 전송</option></select><label className="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2"><FiSearch/><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="직원·장치·경로 검색" className="w-60 bg-transparent text-sm outline-none"/></label></div>
      ...
      {!error && !loading && rows.length > 0 && <div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-slate-950/80 text-xs uppercase text-slate-500"><tr><th className="px-5 py-4">시간</th><th className="px-5 py-4">유형</th><th className="px-5 py-4">직원</th><th className="px-5 py-4">장치</th><th className="px-5 py-4">대상·증거</th><th className="px-5 py-4">심각도</th></tr></thead><tbody className="divide-y divide-slate-800">{rows.map((event) => <tr key={event.id} className="hover:bg-slate-800/50"><td className="whitespace-nowrap px-5 py-4">{new Date(event.occurredAt).toLocaleString()}</td><td className="px-5 py-4 font-medium">{securityEventLabel(event.eventType)}</td><td className="px-5 py-4">{event.employeeName || event.employeeId}</td><td className="px-5 py-4 font-mono text-xs text-slate-400">{event.deviceId}</td><td className="px-5 py-4 font-mono">{event.source || '-'}<SecurityEventDetails details={event.details}/></td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-xs ${event.severity === 'Warning' ? 'bg-amber-500/15 text-amber-400' : 'bg-blue-500/15 text-blue-400'}`}>{event.severity}</span></td></tr>)}</tbody></table></div>}
    </section>
  </div></div>;
}
