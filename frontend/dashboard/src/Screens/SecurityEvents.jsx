...
export default function SecurityEvents() {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-[#1E2939] to-[#0F172A] p-4 text-slate-100 sm:p-6 lg:p-8"><div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/70"><div className="flex flex-col gap-3 border-b border-slate-800 p-4 sm:flex-row sm:items-center sm:justify-between"><select value={filter} onChange={(event) => setFilter(event.target.value)} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2"><option value="">전체 유형</option><option value="USB_CONNECTED">USB 연결</option><option value="USB_DISCONNECTED">USB 해제</option><option value="FILE_COPY">파일 복사</option><option value="FILE_DELETED">파일 삭제</option><option value="FILE_MOVED">파일 이동</option><option value="NETWORK_TRANSFER">네트워크 반출</option></select><label className="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2"><FiSearch/><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="직원·장치·경로 검색" className="w-60 bg-transparent text-sm outline-none"/></label></div>
