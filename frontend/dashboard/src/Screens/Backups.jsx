...
import { canStartInventoryBackup, inventoryBackupButtonLabel, inventoryBackupPercent, inventoryHeartbeat } from '../inventoryProgress';
...
function FolderPolicyRow({ folder, rules, runId, onFolderRule, onFileRule }) {
  ...
  const toggle = async () => {
    ...
    setLoading(true);
    try {
      const query = new URLSearchParams({ folderPath: folder.path, take: '100', skip: '0' });
      setFiles(await request.get(`/backups/inventory/runs/${runId}/files?${query}`) || []);
    } finally { setLoading(false); }
  };
  const applyFileRule = async (file, action) => {
    await onFileRule(file, action);
    setFiles((current) => current.map((value) => value.id === file.id ? { ...value, status: action === 'Include' ? 'Pending' : 'Excluded' } : value));
  };
  return <Fragment><tr><td className="px-4 py-3"><button type="button" onClick={toggle} className="mr-2 inline-flex h-6 w-6 items-center justify-center rounded bg-slate-800 text-xs">{expanded ? '−' : '+'}</button><span style={{ paddingLeft: `${Math.min(folder.depth, 8) * 10}px` }} className="inline-block"><span className="font-medium">{folderDisplayName(folder)}</span><span className="block break-all font-mono text-xs text-slate-500">{folder.path}</span></span></td><td className="whitespace-nowrap px-4 py-3">{folder.fileCount}</td><td className="whitespace-nowrap px-4 py-3">{formatBytes(folder.sizeBytes)}</td><td className="px-4 py-3"><span className={policy.action === 'Exclude' ? 'text-rose-400' : 'text-emerald-400'}>{policy.action === 'Exclude' ? '백업 제외' : '백업 포함'}</span>{policy.inherited && <span className="ml-1 text-xs text-slate-500">(상위/기본)</span>}</td><td className="whitespace-nowrap px-4 py-3"><button onClick={() => onFolderRule(folder, 'Include')} className="mr-2 rounded bg-emerald-500/15 px-2 py-1 text-xs text-emerald-400">폴더 전체 백업</button><button onClick={() => onFolderRule(folder, 'Exclude')} className="rounded bg-rose-500/15 px-2 py-1 text-xs text-rose-400">폴더 전체 제외</button></td></tr>{expanded && <tr><td colSpan="5" className="bg-slate-950/40 px-6 py-4">{loading ? <p className="text-sm text-slate-500">파일을 불러오는 중...</p> : <div className="max-h-72 overflow-auto"><table className="min-w-full text-xs"><thead className="text-slate-500"><tr><th className="px-3 py-2 text-left">하위 파일(최대 100개)</th><th className="px-3 py-2 text-left">크기</th><th className="px-3 py-2 text-left">상태</th><th className="px-3 py-2 text-left">개별 예외</th></tr></thead><tbody>{files.map((file) => <tr key={file.id} className="border-t border-slate-800"><td className="break-all px-3 py-2 font-mono">{file.path}</td><td className="whitespace-nowrap px-3 py-2">{formatBytes(file.sizeBytes)}</td><td className="px-3 py-2">{statusLabel[file.status] || file.status}</td><td className="whitespace-nowrap px-3 py-2"><button onClick={() => applyFileRule(file, 'Include')} className="mr-2 rounded bg-emerald-500/15 px-2 py-1 text-emerald-400">이 파일 포함</button><button onClick={() => applyFileRule(file, 'Exclude')} className="rounded bg-rose-500/15 px-2 py-1 text-rose-400">이 파일 제외</button></td></tr>)}</tbody></table>{!files.length && <p className="py-4 text-center text-slate-500">표시할 파일이 없습니다.</p>}</div>}</td></tr>}</Fragment>;
}
