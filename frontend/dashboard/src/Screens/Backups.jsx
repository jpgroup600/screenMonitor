...
import { formatBytes, newestVersionsFirst } from '../backupFile';
...
export default function Backups() {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-[#1E2939] to-[#0F172A] p-4 text-slate-100 sm:p-6 lg:p-8">
    ...
    {selected && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4" onClick={() => setSelected(null)}><section className="max-h-[85vh] w-full max-w-3xl overflow-auto rounded-2xl border border-slate-700 bg-slate-900 p-6" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2 text-blue-400"><FiArchive/>버전 기록</div><h2 className="mt-2 break-all font-mono text-sm">{selected.originalPath}</h2><p className="mt-2 text-sm text-slate-400">{selected.employeeName} · {selected.deviceId}</p></div><button onClick={() => setSelected(null)} className="p-2 text-slate-400 hover:text-white"><FiX size={22}/></button></div><div className="mt-6 space-y-3">{newestVersionsFirst(selected.versions).map((version) => <article key={version.id} className="rounded-xl border border-slate-800 bg-slate-950/60 p-4"><div className="flex flex-wrap justify-between gap-2"><strong>{new Date(version.uploadedAt).toLocaleString()}</strong><span className="text-slate-400">{formatBytes(version.plainSizeBytes)}</span></div><p className="mt-2 break-all font-mono text-xs text-slate-500">SHA-256 {version.contentHash}</p><p className="mt-1 text-xs text-slate-500">원본 수정: {new Date(version.sourceModifiedAt).toLocaleString()}</p></article>)}</div><p className="mt-6 rounded-lg bg-amber-500/10 p-3 text-sm text-amber-300">복원은 장치별 암호화 키 관리가 준비된 후 제공됩니다.</p></section></div>}
  </div>;
}
